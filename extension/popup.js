...
function init() {
  ...
  chrome.storage.local.get(["kanban", "habits", "_timerState"], (st) => {
    ...
    if (habitsSub) habitsSub.textContent = habits.length ? habitDone === habits.length ? "\u{1F389} all done" : "today" : "no habits";
  ...
  if (destQuick) destQuick.addEventListener("click", () => {
    ...
    if (newFolderBtn) {
      ...
      newFolderBtn.classList.remove("active");
    }
  });
  ...
  if (newFolderBtn) newFolderBtn.addEventListener("click", () => {
    ...
    newFolderBtn.textContent = creatingNewFolder ? "\xD7" : "+";
  ...
  if (saveBtn) saveBtn.addEventListener("click", async () => {
    ...
    if (!title || !currentUrl) {
      showToast("Missing title or URL.", "err");
      return;
    }
    if (currentUrl.startsWith("chrome://") || currentUrl.startsWith("chrome-extension://")) {
      showToast("This page can't be bookmarked.", "err");
    ...
    if (destMode === "quick") {
      if (quickAccess.some((b) => b.url === currentUrl)) {
        showToast("Already in Quick Access.", "err");
        return;
      }
      ...
      if (creatingNewFolder) {
        ...
        if (!folderName) {
          showToast("Enter a folder name.", "err");
          return;
        }
