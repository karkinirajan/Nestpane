...
A worksheet for the CWS Developer Dashboard's **Privacy practices** tab and the Google OAuth consent screen, based on the policy in [`privacy.html`](../privacy.html). Fill these in when submitting Nestpane (and reuse for Edge/Opera add-on stores later — their privacy forms ask nearly identical questions).
...
```
https://<your-github-username>.github.io/nestpane/privacy.html
```
...
These answers are accurate for Nestpane as built — no ads, no analytics, no data sale.

---

## 5. Certification

The Privacy practices tab ends with a certification that you comply with the Developer Program Policies (no deceptive behavior, accurate disclosures, etc.). This is true for Nestpane as documented — check it.

---

## 6. Google OAuth consent screen verification

Nestpane requests `userinfo.email`, `userinfo.profile`, and `drive.appdata` via an "External" OAuth client. Once real users sign in, Google generally requires **OAuth app verification**:
